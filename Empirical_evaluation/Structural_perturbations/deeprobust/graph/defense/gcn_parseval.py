...
class Parseval_GCN(nn.Module):

    def __init__(self, nfeat, nhid, nclass, dropout=0.5, lr=0.01, drop=False, weight_decay=5e-4, n_edge=1,with_relu=True,
                 with_bias=True, scale_param=0.001, num_passes=2,  device=None):

        super(Parseval_GCN, self).__init__()
    ...
    def project_parseval(self):

        param = self.gc1.weight.data
        last = param

        for i in range(self.num_passes):
            temp1 = torch.mm(last.t(), last)
            temp2 = (1 + self.scale_param) * param - self.scale_param *  torch.mm(param, temp1)
            last = temp2

            self.gc1.weight.data = last

        param = self.gc2.weight.data
        last = param
