...
class GCN_noise(nn.Module):
    ...
    def __init__(self, nfeat, nhid, nclass, dropout=0.5, lr=0.01, weight_decay=5e-4,
            with_relu=True, with_bias=True, device=None, noise_ratio_1=0.01, noise_ratio_2=0.01):

        super(GCN_noise, self).__init__()
        ...
        self.noise_ratio_2 = noise_ratio_2
    ...
    def forward(self, x, adj):
        if self.with_relu:
            x = self.gc1(x, adj)
            noise = _get_noise(x, self.noise_ratio_1)
            x = x + noise
            x = F.relu(x)
        else:
            x = self.gc1(x, adj)
        ...
        noise = _get_noise(x, self.noise_ratio_2)
        x = x + noise
